...
## The menu or the dock shows a gear instead of the microphone icon

The desktop matches a running window to its menu entry by name, and versions before 1.1.0 installed the entry under a different name than the window's id; and GTK trusts an icon-theme cache (`~/.local/share/icons/hicolor/icon-theme.cache`, left behind by other installers) until the theme folder itself changes, which hid a renamed icon. The installer now names the icon by its full path in the menu entry, bumps the theme folder, and refreshes a cache where one exists. Update, close the window, and open it again from the menu. If the old entry lingers in the menu, `~/.local/share/applications/usb-audio-transcriber.desktop` can be deleted by hand; a stubborn dock is cured by logging out and in.
