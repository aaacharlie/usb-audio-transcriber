...
- From a terminal: `~/.local/share/usb-audio-transcriber/bin/panel.py open` (`usb-audio-transcriber panel open` with pipx) starts the server if needed and becomes the window, staying in the foreground until it is closed like any desktop app (add `&` to keep the terminal); when the window is already open it raises it and returns. `bin/app.py --page settings` opens straight on a page. Add `--web` for the web page in a browser window (Chrome, Chromium, Brave, Edge) or tab (opened through a one-time link, so the private token stays off the browser's command line), `--browser` for a plain tab, or `--no-browser` to only print the private link.
...
- A private token is created at first start (`var/state/panel-token`, readable by you only). Every request needs it: the window reads it from that file, the private link stores it as a cookie, and the page sends it with each call. Without it the panel shows a short "needs its private link" page and no data. When the panel opens the web page for you it uses a one-time link that expires in a minute, so the token never appears on a browser's command line or in the journal; the long-lived link is printed only by `panel.py url` and `panel.py open --no-browser`, for you to copy.
- Browsers keep cookies per host, not per port, so the panel's cookie is also sent to other web apps you run on `127.0.0.1` (a notebook server, a development server). They would see the token only if they log request headers; use a separate browser profile for the panel if that matters to you.
